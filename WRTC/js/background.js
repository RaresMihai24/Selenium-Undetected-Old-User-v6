...
var WebRTCSetting = function () {
	function WebRTCSetting() {
		...
		chrome.action.onClicked.addListener(this.onBrowserAction.bind(this));
	...
	_createClass(WebRTCSetting, [{
		...
		value: function updateState(addonState) {
			...
			if (_this.state) {
				chrome.action.setTitle({ title: "You are protected" });
			} else {
				chrome.action.setTitle({ title: "Your IP is visible" });
			}
		}
	}, {
		key: 'isControllable',
